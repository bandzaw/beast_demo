...
public:
    ...
    void
    onControlMessage(boost::beast::websocket::frame_type kind,  boost::beast::string_view payload)
    {
...
int main(int argc, char** argv)
{
    ...
    {
        std::cerr <<
            "Usage: websocket-client-async-ssl <host> <port> <text> <cafile> <crtfile> <keyfile>\n" <<
            "Example:\n" <<
    ...
    auto const cafile = argv[4];
    auto const crtfile = argv[5];
    auto const keyfile = argv[6];
    ...
    boost::system::error_code ec;
    loadCertificates(ctx, cafile, crtfile, keyfile, ec);
    if (ec)
    {
        exit(1);
    }
